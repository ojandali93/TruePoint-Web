...
const sortCards = (cards: MasterCard[], by: SortBy): MasterCard[] => {
  ...
  switch (by) {
    ...
    case "rarity":
      return c.sort(
        (a, b) =>
          (RARITY_ORDER[a.rarity ?? ""] ?? 3) -
          (RARITY_ORDER[b.rarity ?? ""] ?? 3),
      );
    case "price":
      return c.sort((a, b) => (b.marketPrice ?? 0) - (a.marketPrice ?? 0));
    default:
      return c;
  }
};
...
const filterCards = (cards: MasterCard[], tab: FilterTab, search: string) =>
  cards.filter((c) => {
    // Hide code cards (digital redemption codes — not real collectibles).
    // Matches the same filter applied on the set browser.
    if ((c.rarity ?? "").toLowerCase().trim() === "code card") return false;
    ...
    if (search) {
      ...
      if (!c.name.toLowerCase().includes(q) && !c.number.includes(q))
        return false;
    }
    return true;
  });
...
const expandForBinder = (
  ...
  for (const card of sorted) {
    ...
    const sortedVariants = [...card.variants].sort(
      (a, b) =>
        (VARIANT_DISPLAY_ORDER[a.variantType] ?? 5) -
        (VARIANT_DISPLAY_ORDER[b.variantType] ?? 5),
    );
  ...
  return (
    <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
      {variants.map((v) => (
        <div
          key={v.variantType}
          title={`${v.label}${v.haveCount > 1 ? ` (×${v.haveCount})` : ""}`}
          style={{
            width: size,
            height: size,
            borderRadius: "50%",
  ...
  return (
    ...
    >
      {slots.map(({ card, variant }, i) => (
        <BinderSlot
          key={`${card.cardId}-${variant.variantType}-${i}`}
          card={card}
          variant={variant}
          onToggle={onToggle}
          size={slotSize}
...
export default function MasterSetDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.get<{
        data: { progress: Progress; cards: MasterCard[] };
      }>(`/master-sets/${setId}`);
  ...
  const handleToggle = async (
    ...
    setProgress((prev) => {
      ...
      return {
        ...prev,
        ownedVariants: prev.ownedVariants + (adding ? 1 : -1),
        needCount: prev.needCount + (adding ? -1 : 1),
        completionPct: Math.round(
          ((prev.ownedVariants + (adding ? 1 : -1)) / prev.totalVariants) * 100,
        ),
      };
    });

    try {
      await api.post(`/master-sets/${setId}/cards/${cardId}/toggle`, {
        variantType,
      });
    } catch {
      // Revert on error
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      >
        <button
          onClick={() => router.push("/master-sets")}
          style={{
            border: "none",
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
          ...
          {progress?.logoUrl && (
            <img
              src={progress.logoUrl}
              alt={progress?.setName}
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ position: "relative", marginBottom: 6 }}>
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: `${progress?.completionPct ?? 0}%`,
                    background: `linear-gradient(90deg, #C9A84C, ${statusColor})`,
                    borderRadius: 4,
            ...
            <div style={{ display: "flex", gap: 20, fontSize: 12 }}>
              ...
              <span style={{ color: "var(--text-dim)" }}>
                {progress?.needCount} Needed
              </span>
              {(progress?.dupeCount ?? 0) > 0 && (
                <span style={{ color: "#F59E0B" }}>
                  {progress?.dupeCount} Dupes
                </span>
              )}
            </div>
          </div>
        ...
        {(["number", "name", "rarity", "price"] as SortBy[]).map((s) => (
          <button
            ...
            style={{
              ...
              background:
                sortBy === s ? "rgba(201,168,76,0.1)" : "var(--surface-2)",
              color: sortBy === s ? "var(--gold)" : "var(--text-dim)",
              fontSize: 11,
              cursor: "pointer",
              fontFamily: "inherit",
          ...
          {(["grid", "table", "binder"] as ViewMode[]).map((v) => (
            <button
              ...
              style={{
                ...
                border: "none",
                borderRight:
                  v !== "binder" ? "1px solid var(--border)" : "none",
                background:
                  view === v ? "rgba(201,168,76,0.12)" : "var(--surface-2)",
          ...
          {(
            ...
            <button
              ...
              style={{
                padding: "12px 16px",
                border: "none",
                borderBottom: `2px solid ${filterTab === key ? "var(--gold)" : "transparent"}`,
                background: "transparent",
                color: filterTab === key ? "var(--gold)" : "var(--text-dim)",
                fontSize: 12,
                cursor: "pointer",
                fontFamily: "inherit",
                fontWeight: filterTab === key ? 500 : 400,
              }}
            >
              {label}
            </button>
            ...
            {([9, 12, 4, 16] as PocketSize[]).map((p) => (
              <button
                ...
                style={{
                  padding: "6px 12px",
                  border: "none",
                  borderRight: p !== 16 ? "1px solid var(--border)" : "none",
                  background:
                    pocketSize === p ? "rgba(201,168,76,0.12)" : "transparent",
                  color: pocketSize === p ? "var(--gold)" : "var(--text-dim)",
                  fontSize: 11,
                  cursor: "pointer",
                  fontFamily: "inherit",
                }}
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                height: 20,
                borderRadius: 10,
                background: stackVariants ? "#10B981" : "var(--border)",
                cursor: "pointer",
          ...
          >
            {displayCards.map((card) => (
              <GridCard
                key={card.cardId}
                card={card}
                onToggle={(variantType) =>
                  handleToggle(card.cardId, variantType)
                }
              />
            ))}
          </div>
        ...
        {!loading && view === "table" && (
          ...
          >
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "50px 1fr 80px 100px 80px 120px 80px",
                gap: 0,
                padding: "8px 16px",
                background: "var(--surface-2)",
                borderBottom: "1px solid var(--border)",
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleToggle(
                      card.cardId,
                      card.variants[0]?.variantType ?? "normal",
                    )
                  }
                  style={{
                    padding: "4px 10px",
                    ...
                    background: "transparent",
                    color:
                      card.ownedVariants > 0 ? "#10B981" : "var(--text-dim)",
                    fontSize: 10,
                    cursor: "pointer",
                    fontFamily: "inherit",
                  }}
            ...
            >
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  cursor: binderPage === 0 ? "not-allowed" : "pointer",
                  fontFamily: "inherit",
                  opacity: binderPage === 0 ? 0.4 : 1,
                }}
              >
                ← Prev
              </button>
